...
class CamYnet(nn.Module):
    ...
    def forward_train_step1(self, s, t):
        ...
        s_u1c = self.u5c(s, c4s)
        s_u2c = self.u6c(s_u1c, c3s)
        s_u3c = self.u7c(s_u2c, c2s)
        s_u4c = self.u8c(s_u3c, c1s)
        s_uc = self.cec(s_u4c)
        s_uc = F.sigmoid(s_uc)

        s_u1m = self.u5m(s, c4s)
        s_u2m = self.u6m(s_u1m, c3s)
        s_u3m = self.u7m(s_u2m, c2s)
        s_u4m = self.u8m(s_u3m, c1s)
        s_um = self.cem(s_u4m)
        s_um = F.sigmoid(s_um)
        ...
        return s_u, s_uc, s_um, [c1s, c2s, c3s, c4s, s, s_u1, s_u2, s_u3, s_u4], [c1t, c2t, c3t, c4t, t, t_u1, t_u2, t_u3, t_u4], 
    ...
    def forward_train_step2(self, s, t):
        ...
        s_uc = self.u5c(s, c4s)
        s_uc = self.u6c(s_uc, c3s)
        s_uc = self.u7c(s_uc, c2s)
        s_uc = self.u8c(s_uc, c1s)
        s_uc = self.cec(s_uc)
        s_uc = F.sigmoid(s_uc)

        s_um = self.u5m(s, c4s)
        s_um = self.u6m(s_um, c3s)
        s_um = self.u7m(s_um, c2s)
        s_um = self.u8m(s_um, c1s)
        s_um = self.cem(s_um)
        s_um = F.sigmoid(s_um)
    ...
    def forward_train_combine(self, s, t):
        ...
        t_d = self.cea(t_d)
        t_d = F.sigmoid(t_d)
        return s_u, s_uc, s_um, s_d, t_d, [c1s, c2s, c3s, c4s, s, s_u1, s_u2, s_u3, s_u4], [c1t, c2t, c3t, c4t, t, t_u1, t_u2, t_u3, t_u4]

    def forward_test(self, s):
        s, c1, _ = self.c1(s)
        s, c2, _ = self.c2(s)
        s, c3, _ = self.c3(s)
        s, c4, _ = self.c4(s)
        _, s, _ = self.cu(s) # no maspool for U bottom
        s_u = self.u5(s, c4)
        s_u = self.u6(s_u, c3)
        s_u = self.u7(s_u, c2)
        s_u = self.u8(s_u, c1)
        ...
        s_u1c = self.u5c(s, c4)
        s_u2c = self.u6c(s_u1c, c3)
        s_u3c = self.u7c(s_u2c, c2)
        s_u4c = self.u8c(s_u3c, c1)
        s_uc = self.cec(s_u4c)
        s_uc = F.sigmoid(s_uc)

        s_u1m = self.u5m(s, c4)
        s_u2m = self.u6m(s_u1m, c3)
        s_u3m = self.u7m(s_u2m, c2)
        s_u4m = self.u8m(s_u3m, c1)
        s_um = self.cem(s_u4m)
        s_um = F.sigmoid(s_um)

        return s_u, s_uc, s_um
